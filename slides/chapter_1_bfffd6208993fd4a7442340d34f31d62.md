...
`@script`
From your introductory SQL classes, you would remember the `group by` clause. We also discussed the use of `CASE` statements in previous lessons in this course. In this lesson, we will begin combining these statements to produce more advanced SQL queries!
...
To produce this output, we can write a SELECT statement that includes two aggregate CASE statements columns. Here, in each aggregate CASE statement, we set a value of 1 if the particular person is of the required sex, and then we add all these 1s together to get the total number of people of that sex within each group. This gives us our required output above!
...
`@script`
We can similarly keep adding more descriptive columns with these case statements. Here, we also include columns that give information about the number of siblings/spouses each passenger had on the ship, with two more SUM - CASE aggregate columns.
...
For example, in the Titanic data, we also have access to the age of each passenger. How do we calculate the average male and female age of passengers in each "pclass"? 
...
`@script`
We can also combine the grouped case statements with counting functions. For example, we also have ticketing and port information about our Titanic passengers. Could we use these methods to find the number of unique tickets issued for passengers in each class, at each embarked port?
...
`@script`
Yes! We can combine count, distinct, and case keywords, to count the number of distinct tickets at each embarked port, as shown here.
